...
function cadastraF(){
    ...
    let farmacias = []
        let objeto = {
            codFarmacia: parseInt(prompt(`Informe o código da farmácia`)),
        ...
        if (remedios.some( (item) =>
            (item.codFarmacia == venda.farmacia && item.remedio == venda.remedio) )){
                ...
                if (venda.qtde <= remedios[indice].qtde){
                    ...
                    document.getElementById("resultado").innerHTML = 'Venda realizada com sucesso'
                }
                else {
                    document.getElementById("resultado").innerHTML = 'Estoque insuficiente' 
                }
        }
        else {
            document.getElementById("resultado").innerHTML = 'Farmácia ou remédio inexistente'
        }
}
